    ...
    <div className="space-y-3 p-3 lg:p-4">
      ...
      {canEditInline && isEditingTitle ? (
        ...
        >
          ...
          {canEditInline && (
            <Edit3 className="h-3 w-3 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity mt-0.5 shrink-0" />
          )}
        </div>
      )}
        ...
        {!isPreviewMode && !isClient ? (
          <Popover>
            <PopoverTrigger asChild>
              <button>
                <Badge className={`text-[9px] font-bold h-5 cursor-pointer ${statusColors[task.status] || "bg-muted"}`}>
          ...
          <Badge className={`text-[9px] font-bold h-5 ${statusColors[task.status] || "bg-muted"}`}>
            {statusLabels[task.status] || task.status}
          </Badge>
        )}
        {canEditInline ? (
          <Popover>
            <PopoverTrigger asChild>
              <button>
                <Badge
                  className={`text-[9px] font-bold h-5 border cursor-pointer ${priorityColors[task.priority] || ""}`}
                >
          ...
          <Badge className={`text-[9px] font-bold h-5 border ${priorityColors[task.priority] || ""}`}>
            {priorityLabels[task.priority] || task.priority}
          </Badge>
        )}
        {hasNoAssignment && (
          <Badge className="bg-destructive text-destructive-foreground text-[9px] h-5">NIEPRZYPISANE</Badge>
        )}
      ...
      {!isPreviewMode && !isClient && (
        <div className="space-y-1.5">
              ...
              {(assignments || []).map((a: any) => (
                <div key={a.user_id} className="relative group/avatar">
                  <Avatar
                    ...
                    title={`${a.profiles?.full_name} (${roleLabels[a.role] || a.role})`}
                  >
              ...
              <PopoverContent className="w-52 p-2" align="start">
                <div className="max-h-40 overflow-y-auto space-y-0.5">
                  ...
                  {unassignedProfiles.length === 0 && (
                    <p className="text-[10px] text-muted-foreground py-1">Wszyscy przypisani</p>
                  )}
                </div>
              </PopoverContent>
            </Popover>
          ...
          {assignments?.length === 0 && (
            <p className="text-[10px] text-destructive font-medium">Brak przypisanych osób</p>
          )}
        </div>
      )}
              ...
              <PopoverContent className="w-52 p-0" align="end">
                <Command>
                  ...
                  <CommandList>
                    ...
                    <CommandGroup>
                      <CommandItem onSelect={() => handleClientChange(null)} className="text-muted-foreground text-xs">
        ...
        <div className="rounded-md border border-destructive/40 bg-destructive/5 p-2 space-y-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-semibold text-destructive">
              {(task as any).bug_severity === "critical" ? "Krytyczny błąd" : "Błąd"}
            </span>
          </div>
        ...
        <>
          ...
          <div className="space-y-1">
            ...
            {(task.status === "review" || task.status === "corrections") && (
              <Button
                size="sm"
                className="w-full justify-start text-[11px] gap-1.5 h-7 bg-amber-500 hover:bg-amber-600 text-white"
                onClick={() => {
                  if (totalLogged <= 0) {
                    toast.error(
                      "Brak zalogowanego czasu. Musisz zalogować czas pracy przed wysłaniem zadania do klienta.",
                    );
                    return;
                  }
            ...
            {task.status === "review" && (
              <Button
                variant="destructive"
                size="sm"
                className="w-full justify-start text-[11px] gap-1.5 h-7"
            ...
            {!["review", "client_review", "client_verified", "done", "closed", "cancelled"].includes(
              task.status || "",
            ) && (
              <Button
                variant="outline"
                size="sm"
                className="w-full justify-start text-[11px] gap-1.5 h-7 border-emerald-500/50 text-emerald-600 hover:bg-emerald-500/10"
                onClick={async () => {
                  try {
            ...
            {(task.status === "in_progress" || task.status === "todo") && !(task as any).is_misunderstood && (
              <Button
                variant="outline"
                size="sm"
                className="w-full justify-start text-[11px] gap-1.5 h-7 border-amber-500/50 text-amber-600 hover:bg-amber-500/10"
      ...
      {isClient && task.status === "client_review" && (
        <>
          <Separator className="my-2" />
          <ClientReviewSection
            clientReviewOpen={clientReviewOpen}
            setClientReviewOpen={setClientReviewOpen}
            correctionSeverity={correctionSeverity}
            setCorrectionSeverity={setCorrectionSeverity}
            correctionText={correctionText}
            setCorrectionText={setCorrectionText}
            handleClientAccept={handleClientAccept}
      ...
      {!isClient && (
        <>
          ...
          {isPreviewMode ? (
            <Button
              size="sm"
              className="w-full text-[11px] h-7 bg-amber-400 hover:bg-amber-300 text-amber-950 font-bold"
            ...
            <Button
              variant="outline"
              size="sm"
              className="w-full text-[11px] gap-1.5 h-7"
      ...
      <div className="flex items-start gap-1.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-semibold text-amber-700 dark:text-amber-400">
            {reporter?.full_name || "Pracownik"} zgłosił niezrozumienie
          </p>
          {task.misunderstood_reason && (
            <p className="text-[10px] text-muted-foreground mt-0.5">„{task.misunderstood_reason}"</p>
          )}
        </div>
      </div>
      <Button variant="outline" size="sm" className="text-[10px] h-5 w-full" onClick={onResolve}>
  ...
  if (!clientReviewOpen) {
    return (
      <div className="space-y-1">
        ...
        <Button
          size="sm"
          className="w-full text-[11px] h-7 gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white"
        ...
        <Button
          ...
          onClick={() => setClientReviewOpen(true)}
        >
          <AlertTriangle className="h-3 w-3" />
          Do poprawy
        </Button>
  ...
  return (
    <div className="space-y-2">
      ...
      <Textarea
        ...
        onChange={(e: any) => setCorrectionText(e.target.value)}
        placeholder="Co wymaga poprawy?"
