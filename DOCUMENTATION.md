...
- **Tworzenie:** Dialog z polami: tytuł, opis, priorytet, typ, klient, projekt, data, czas, brief, przypisane osoby. Pola relacyjne (Klient, Projekt, Przypisane osoby) korzystają z **wyszukiwalnych komponentów Combobox** (shadcn/ui Command) — umożliwiają filtrowanie przez wpisanie tekstu, co znacząco ułatwia pracę na dużych zbiorach danych, szczególnie na urządzeniach mobilnych. Pole Klient/Projekt to single-select, Przypisane osoby to multi-select z chipami. **Tworzenie klienta po NIP:** pole NIP z przyciskiem "Znajdź" — wyszukuje klienta w bazie po NIP, a jeśli nie istnieje, pobiera dane z API MF (Biała Lista VAT) i automatycznie tworzy nowego klienta w tabeli `clients`.
...
### 7.4 Flaga "Niezrozumiałe zadanie" (Misunderstood Task)

**Przepływ:**
1. Pracownik (rola staff) klika "Nie rozumiem polecenia" w widoku zadania → modal z polem tekstowym (powód).
2. System ustawia `is_misunderstood = true`, `misunderstood_by = user_id`, `misunderstood_reason = tekst`.
3. Zdarzenie logowane w `activity_log` (action: `misunderstood_reported`).
4. Komentarz wewnętrzny dodawany automatycznie z treścią zgłoszenia.

**UI:**
- **Kanban:** Karta z `is_misunderstood = true` otrzymuje żółte tło (`bg-amber-500/10`, `border-amber-500/30`) i badge "⚠️ Niezrozumiałe".
- **Szczegóły zadania:** Żółty banner z imieniem pracownika, treścią powodu i przyciskiem "Oznacz jako wyjaśnione". Banner UKRYTY przed klientem.
- **Rozwiązanie:** Kliknięcie "Oznacz jako wyjaśnione" resetuje `is_misunderstood` na false, czyści `misunderstood_by` i `misunderstood_reason`, loguje w `activity_log` (action: `misunderstood_resolved`).
- **Historia:** Oba zdarzenia (zgłoszenie i rozwiązanie) widoczne na osi czasu w StatusTimeline.
- Alert w TaskAlertBanners z licznikiem.

### 7.5 Widoczność komentarzy dla klienta

Klient widzi wszystkie komentarze z `type !== "internal"` (typ: "client" lub inne zewnętrzne). Może również dodawać własne komentarze zewnętrzne. Wcześniej klient widział tylko komentarze z flagą `requires_client_reply = true` — ta logika została rozszerzona.
